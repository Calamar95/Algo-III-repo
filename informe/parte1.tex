...
\textbf{ES CORRECTO LO SIGUIENTE????::}

Dedujimos que el algoritmo indica cuantas ciudades se pueden conectar, para cada ramal en O(n), siendo n la cantidad de estaciones en cada ramal, ya que el primer while se ejecuta como mucho n veces, el segundo while se ejecuta también n veces, y dentro de él hay otro while que se ejecuta longitud del cable veces \textbf{ó n veces tambien?????.}
Además usamos las funciones $>$, $<$, $\geq$, $+$, $-$, $==$,  de complejidad constante, la asignación, también O(1), el $operator[]$ de complejidad O(1) y la función del vector $.back()$ que es también O(1).
