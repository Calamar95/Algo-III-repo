...
El algoritmo es correcto porque empieza viendo cuantas ciudades se conectan desde la ciudad en el km 0, sunpongamos que llega hasta la ciudad k la cual no es la ultima, pues si o fuera el algoritmo ya hubiera temrinado. Entonces tenemos k+1 ciudades conectadas. Luego quita la ciudad en el km0 y añade al cable la distancia entre esa ciudad y la siguiente, (ciudad 1) , suma esa dijferencia a lo que habia sobrado de cable y se fija si se pueden conectar mas ciudades, continuando desde la ciudad k, cuando ya no se pueden conectar mas ciudades, se fija cuantas conecto desde la ciudad uno hasta la ultima ciudad que pudo conectar y si el numero de ciudades conectadas es mayor a k+1 se guarda este nuevo valor. El proceso re repite hasta que se llega a la ultima ciudad. Cuando se llega a la ultima ciudad, el algoritmo devuelve el valor guardado con la cantidad maxima de ciudades que se puedieron conectar en algun momento
...
\minipage{0.8\textwidth}
  \includegraphics[width=\linewidth]{../graficos/ej1/CableLargoo.png}
